  ...
  const time = async (ms) => {
    return new Promise(resolve => setTimeout(resolve, ms));
        ...
        if (warn < 2) {
            ...
            m.reply('Kamu telah diperingatkan oleh admin, dan sekarang kamu punya ' + (warn + 1) + ' warn . Ingat Jika kamu mendapat warn 3 kali kamu akan otomatis diban dari Grup', ban)
        } else if (warn == 2) {
            //global.DATABASE._data.users[ban].banned = true
            global.DATABASE._data.users[ban].warn = 0
            conn.fakeReply(m.chat, 'Selamat Jalan tod', '0@s.whatsapp.net', `${conn.user.name} Verified Bot`, 'status@broadcast')
                await time(5000)
             await conn.groupRemove(m.chat, [ban])
             m.reply('*Lu diban karena telah mendapat 3 kali warn*', ban)
